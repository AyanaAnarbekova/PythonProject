...
class Spaceship(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.remaining_health > 0:
            ...
            self.rect.x, (self.rect.bottom + 5), int((self.rect.width) * (self.remaining_health / self.initial_health)), 10))
...
def create_aliens():
    for row in range(1, rows_aliens):
...
for shield in range(shields_num):
    for row in range(5):
