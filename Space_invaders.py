...
img6 = pygame.transform.scale(boss, (400, 400))
...
class Spaceship(pygame.sprite.Sprite):
    ...
    def shoot(self):
        ...
        if len(spaceship_group) > 0 and len(lazer_group) <= 3:
...
class Boss(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.centerx = int(width/2)
        self.rect.centery = int(height/2 - 100)
        self.health = 10
        self.remaining_health = 10
    ...
    while play_game1:
        ...
        if level < 5:
            ...
            if time_now - last_alien_shoot > alien_cooldown:
                boss = Boss()
                boss_group.add(boss)
                if spaceship.rect.x >= 175 or spaceship.rect.x <= 575:
